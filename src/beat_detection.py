...
This script analyzes an audio file using the `librosa` library to extract key
musical features:
- **Tempo (BPM)**: Determines the speed of the music.
- **Beat Times**: Detects rhythmic beats.
- **Onset Times**: Identifies sharp sound changes
- **RMS Energy**: Measures loudness over time.
- **Chroma Features**: Analyzes pitch (musical notes present over time).

The extracted features can be used to synchronize lights with music.
...
def analyze_audio(y: np.ndarray, sr: int) -> Dict[str, np.ndarray]:
    ...
    Returns:
        Dict[str, np.ndarray]: Dict containing extracted audio features:
            - "tempo" (np.ndarray): Estimated beats per minute (BPM).
            - "beat_times" (np.ndarray): Array of times (seconds) when beats
              occur.
            - "onset_times" (np.ndarray): Array of times (seconds) when musical
              onsets occur.
            - "rms" (np.ndarray): Normalized root mean square (RMS) energy
              levels.
            - "chroma" (np.ndarray): Transposed chroma matrix (time, pitch
              class).
    """
